...
function createArticle(data) {
  ...
  articleContent.forEach(function(c) {
    return article.appendChild(c);
  });
  article.appendChild(articleButton);
  /*Event listener*/
  var isOpen = false;
  var slideAnim = TweenMax.to(article, 1, {
    height: '400px',
  }).reverse();
...
function createForm(data) {
  ...
  var formContentBox = document.createElement('textarea');
...
if (formElement) {
  formElement.addEventListener('submit', function(event) {
    ...
    var text = elements[2].value.split('\n');
    var article = {
